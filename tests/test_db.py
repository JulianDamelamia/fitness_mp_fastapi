...
def test_create_routine_with_sessions(db_session):
    ...
    s1 = Session(name="Día 1 - Push")
    s2 = Session(name="Día 2 - Pull")
    s3 = Session(name="Día 3 - Legs")
    ...
    assert rutina.sessions[0].name in ["Día 1 - Push", "Día 2 - Pull"]
    ...
    result = (
        ...
        .filter(Routine.sessions.any(Session.name == "Día 1 - Push"))
        .first()
    )
