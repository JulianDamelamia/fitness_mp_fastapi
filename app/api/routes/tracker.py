...
from app.models.fitness import Session as FitnessSession, Routine
...
def start_session_tracking(
    ...
    has_purchased_access = (
        ...
        .first()
    )  # .first() es más rápido, solo nos importa si existe (1) o no (None)
    ...
    return templates.TemplateResponse(
        "track_session.html", {"request": request, "session": session}
    )
...
async def log_session(
    ...
    session_template = (
        ...
        .filter(FitnessSession.id == session_template_id)
        .first()
    )
    ...
    new_session_log = SessionLog(
        date=datetime.now(), session_id=session_template.id, user_id=current_user.id
    )
...
def get_tracking_history(
    ...
    logs = (
        ...
        .options(
            ...
            joinedload(SessionLog.exercise_logs).joinedload(
                ExerciseLog.exercise
            ),  # Carga los sets Y el nombre del ejercicio
        )
        .all()
    )

    return templates.TemplateResponse(
        "history.html", {"request": request, "logs": logs}
    )
