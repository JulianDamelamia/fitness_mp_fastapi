...
def new_routine_page(
    ...
    sessions_models = db.query(FitnessSession).filter(
    ...
    return templates.TemplateResponse("routine_form.html", {
        "request": request,
...
def get_routine_detail(
    ...
    if routine.creator_id != current_user.id:
        raise HTTPException(status_code=403, detail="No tienes permisos para ver esta rutina")
