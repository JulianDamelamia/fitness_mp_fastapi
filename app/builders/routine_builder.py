    ...
    def create_exercise(db, session, ex_data):
        ...
        for field in required_fields:
            if getattr(ex_data, field, None) is None:
                raise ValidationError(f"El campo '{field}' es obligatorio para ejercicios nuevos")
...
class SessionBuilder:
    ...
    def create_session(session_name: str, exercise_list: List, db):
        ...
        if not session_name or not session_name.strip():
            raise ValidationError("El campo 'session_name' es obligatorio y no puede estar vacío")

        session = Session(session_name=session_name)
        for e in exercise_list:
            exercise = ExerciseBuilder.create_exercise(db=db, session=session, ex_data=e)
